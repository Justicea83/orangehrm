 ...
 */
import { required, afterDate, endDateShouldBeAfterStartDate, validEmailFormat, validPhoneNumberFormat, endTimeShouldBeAfterStartTime, startDateShouldBeBeforeEndDate, startTimeShouldBeBeforeEndTime, minValueShouldBeLowerThanMaxValue, maxValueShouldBeGreaterThanMinValue, numberShouldBeBetweenMinAndMaxValue, digitsOnly, digitsOnlyWithDecimalPoint, digitsOnlyWithDecimalPointAndMinusSign, shouldBeCurrentOrPreviousDate, validHexFormat, imageShouldHaveDimensions, greaterThanOrEqual, lessThanOrEqual, validLangString, validSelection, validHostnameFormat, validPortRange, validVideoURL, } from '../rules';
jest.mock('@/core/plugins/i18n/translate', () => {
    const t = (langString) => {
        const mockStrings = {
            'general.required': 'Required',
            'general.to_date_should_be_after_from_date': 'To date should be after From date',
            'general.allows_phone_numbers_only': 'Allows numbers and only + - / ( )',
            'general.start_time_should_be_before_end_time': 'Start time should be before end time',
            'general.end_time_should_be_after_start_time': 'End time should be after start time',
            'general.start_date_should_be_before_end_date': 'Start date should be before end date',
            'general.end_date_should_be_after_start_date': 'End date should be after start date',
            'general.expected_email_address_format_not_matched': 'Expected format: admin@example.com',
            'general.should_be_a_number': 'Should be a number',
            'general.should_be_a_number_between_min_and_max': 'Should be a number between 0-100',
            'general.invalid': 'Invalid',
            'general.incorrect_dimensions': 'Incorrect Dimensions',
            'general.less_than_or_equal_to_n': 'Number should be less than or equal to 100',
            'general.greater_than_or_equal_to_n': 'Number should be greater than or equal to 0',
            'general.enter_valid_port_between_a_to_b': 'Enter a valid port number between 0 to 65535',
            'general.invalid_video_url_message': 'This URL is not a valid URL of a video or it is not supported by the system',
        };
        return mockStrings[langString];
    };
    return { translate: jest.fn(() => t) };
});
const createMockFile = (type = 'image/gif', base64 = 'R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7', name = 'mock', size = 100) => {
    return {
        name,
        type,
        size,
...
describe('core/util/validation/rules::endDateShouldBeAfterStartDate', () => {
    ...
    test('endDateShouldBeAfterStartDate::invalid case (custom message)', () => {
        const result = endDateShouldBeAfterStartDate('2021-06-29', 'To date should be after From date')('2021-06-28');
...
describe('core/util/validation/rules::endTimeShouldBeAfterStartTime', () => {
    ...
    test('endTimeShouldBeAfterStartTime:: should return custom message on invalid time', () => {
        const result = endTimeShouldBeAfterStartTime('08:00', 'Invalid time')('07:00');
...
describe('core/util/validation/rules::startDateShouldBeBeforeEndDate', () => {
    ...
    test('startDateShouldBeBeforeEndDate::valid (start date as function)', () => {
        const result = startDateShouldBeBeforeEndDate(() => '2021-10-25')('2021-10-28');
        expect(result).toBeTruthy();
    });
    test('startDateShouldBeBeforeEndDate::invalid case (custom message)', () => {
        const result = startDateShouldBeBeforeEndDate('2021-10-28', 'From date should be before To date')('2021-10-31');
...
describe('core/util/validation/rules::startTimeShouldBeBeforeEndTime', () => {
    ...
    test('startTimeShouldBeBeforeEndTime:: should return custom message on invalid time', () => {
        const result = startTimeShouldBeBeforeEndTime('07:00', 'Invalid time')('08:00');
        expect(result).toEqual('Invalid time');
    });
...
describe('core/util/validation/rules::maxValueShouldBeGreaterThanMinValue', () => {
    test('maxValueShouldBeGreaterThanMinValue:: should not allow minimum value to be greater than maximum value', () => {
        const result = maxValueShouldBeGreaterThanMinValue('100', 'Should be higher than Minimum Salary')('1');
    ...
    test('maxValueShouldBeGreaterThanMinValue:: should allow minimum value to be lower than maximum value', () => {
        const result = maxValueShouldBeGreaterThanMinValue('100', 'Should be higher than Minimum Salary')('101');
        expect(result).toEqual(true);
    });
...
describe('core/util/validation/rules::minValueShouldBeLowerThanMaxValue', () => {
    test('minValueShouldBeLowerThanMaxValue:: should not allow minimum value to be greater than maximum value', () => {
        const result = minValueShouldBeLowerThanMaxValue('10', 'Should be lower than Maximum Rating')('100');
    ...
    test('minValueShouldBeLowerThanMaxValue:: should allow minimum value to be lower than maximum value', () => {
        const result = minValueShouldBeLowerThanMaxValue('100', 'Should be lower than Maximum Rating')('10');
        expect(result).toEqual(true);
    });
...
describe('core/util/validation/rules::numberShouldBeBetweenMinAndMaxValue', () => {
    ...
    test('numberShouldBeBetweenMinAndMaxValue:: should display custom message', () => {
        const result = numberShouldBeBetweenMinAndMaxValue(0, 10, 'Rating should be between 0 and 10')('11');
...
describe('core/util/validation/rules::imageShouldHaveDimensions', () => {
    ...
    const mockDomImage = (width, height) => {
    ...
    test('imageShouldHaveDimensions:: ignore SVG size validation', async () => {
        const result = await imageShouldHaveDimensions(50 / 50)(createMockFile('image/svg+xml', '', 'mock.svg'));
        expect(result).toStrictEqual(true);
    });
});
...
describe('core/util/validation/rules::validPortRange', () => {
    ...
    test('validPortRange:: with port value out of range', () => {
        ...
        expect(result).toStrictEqual('Enter a valid port number between 0 to 65535');
    });
...
describe('core/util/validation/rules::validVideoURL', () => {
    ...
    test('validVideoURL:not a url', () => {
        ...
        expect(result).toBe('This URL is not a valid URL of a video or it is not supported by the system');
    });
