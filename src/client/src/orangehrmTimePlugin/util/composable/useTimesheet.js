...
export default function useTimesheet(http, date, empNumber) {
    const { state, fetchTimesheet, updateTimesheet, fetchTimesheetEntries } = useTimesheetAPIs(http);
    ...
    const timesheetPeriod = computed(() => {
        ...
        if (!startDate || !endDate)
            return null;
        return `${formatDate(startDate, jsDateFormat, { locale })} ${$t('general.to').toLowerCase()} ${formatDate(endDate, jsDateFormat, { locale })}`;
    });
    return {
        state,
        onClickEdit,
...
//# sourceMappingURL=useTimesheet.js.map