...
import { OxdButton, OxdCardTable, OxdDivider, OxdForm, OxdFormActions, OxdFormRow, OxdGrid, OxdGridItem, OxdInputField, OxdPagination, OxdTableFilter, } from '@ohrm/oxd';
...
const MESSAGE_DELETE = 'The selected record will be permanently deleted. Are you sure you want to continue?';
const MESSAGE_APPROVE = 'The selected assignment will be approved. Are you sure you want to continue?';
const MESSAGE_REJECT = 'The selected assignment will be rejected. Are you sure you want to continue?';
export default {
    name: 'TaskGroups',
    components: {
        AddCommentModal,
    ...
    setup(props) {
        ...
        const { assignmentActions, performAssignmentAction } = useAssignmentActions(http);
        const { showPaginator, currentPage, total, pages, response, isLoading, execQuery, } = usePaginate(http, {
    ...
    methods: {
        actionCellRenderer(...[, , , row]) {
            ...
            more.props.onClick = ($event) => this.onAssignmentDropdownAction($event, row);
    ...
    mounted() {
        ...
        this.emitter.on('approve', (taskGroup) => {
            ...
            this.$refs.deleteDialog.showDialog().then((confirmation) => {
                if (confirmation === 'ok') {
                    this.performAssignmentAction({ action: 'approve', groupAssignmentId: taskGroup.id });
                }
            });
        });
