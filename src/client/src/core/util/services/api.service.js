            ...
            this._http.interceptors.response.use((response) => {
                ...
                if (config.url && headers) {
                    ...
                    if (etag && etag !== cachedEtag) {
                        ...
                        this._cacheStorage.setItem(url, removeETagWeakValidatorDirective(etag));
                        if (cachedEtag)
                            this._cacheStorage.removeItem(cachedEtag);
                        this._cacheStorage.setItem(removeETagWeakValidatorDirective(etag), JSON.stringify(response.data));
                    }
                }
                return response;
            }, (error) => {
                if (error.response?.status === 304) {
                    ...
                    if (etag) {
                        const cacheData = this._cacheStorage.getItem(removeETagWeakValidatorDirective(etag));
                        if (cacheData) {
